...
>(
    ...
    if (!component) return undefined as C extends undefined ? undefined : CrudRoute<P, T, NonNullable<C>, Meta>;

    return {path, name, component, meta} as C extends undefined ? undefined : CrudRoute<P, T, NonNullable<C>, Meta>;
};
...
>(
    ...
    const children: ParentCrudRoute<
        ...
        ShowComponent
    >['children'] = [
        createStandardRouteConfig('', `${baseRouteName}${OVERVIEW_PAGE_NAME}`, components.overview, meta),
        createStandardRouteConfig('create', `${baseRouteName}${CREATE_PAGE_NAME}`, components.create, meta),
        createStandardRouteConfig(':id/edit', `${baseRouteName}${EDIT_PAGE_NAME}`, components.edit, meta),
...
>(
    ...
    components: {overview: OverviewComponent; create: CreateComponent; edit: EditComponent; show: ShowComponent},
    meta: Meta = {} as Meta,
): NestedParentCrudRoute<N, Meta, OverviewComponent, CreateComponent, EditComponent, ShowComponent> => {
    // @ts-expect-error FilterUndefined is a compile-time tuple filter, but .filter() produces a generic array
